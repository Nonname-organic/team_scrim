...
export async function POST(req: NextRequest) {
  ...
  try {
    const { match_ids, map_filter } = await req.json()

    const context = await buildAIContextV2(auth.teamId, {
...
export async function GET() {
  ...
  const reports = await query(
    `SELECT * FROM ai_reports WHERE team_id = $1 ORDER BY created_at DESC LIMIT 10`,
    [auth.teamId]
  )
