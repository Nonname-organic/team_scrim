...
export async function POST(req: NextRequest) {
  ...
  let body: { team_id?: string; match_ids?: string[]; map_filter?: string }
  try { body = await req.json() } catch {
    return NextResponse.json({ error: 'Invalid request body' }, { status: 400 })
  }

  const { team_id, match_ids, map_filter } = body
    ...
    async start(controller) {
      ...
      try {
        ...
        if (!context.match_details.length) {
          send({ error: 'データが見つかりません。試合データを追加してから分析してください。' })
          controller.close()
          return
        }
        ...
        if (!parsedReport) {
          console.error('[AI] Failed to parse JSON:', fullText.slice(0, 500))
          send({ error: 'AIの応答をJSON形式で解析できませんでした。再度お試しください。' })
          controller.close()
          return
        }
        ...
        send({ done: true, data: { ...saved[0], ...parsedReport, raw_analysis: fullText } })
      } catch (err) {
