...
export async function GET(
  ...
  try {
    ...
    const playerCheck = await queryOne('SELECT id FROM players WHERE id = $1 AND team_id = $2', [playerId, auth.teamId])
    if (!playerCheck) return notFoundError('選手が見つかりません')
...
export async function POST(
  ...
  try {
    ...
    return serverError('players/[id]/stats POST', err)
  }
}
